...
@Directive({
  selector: `input[autocomplete], textarea[autocomplete]`,
  host: {
  ...
  implements ControlValueAccessor, AfterViewInit, OnChanges, OnDestroy
{
   ...
   */
  @Input('autocompletePosition') position: 'auto' | 'above' | 'below' = 'auto';

  /**
   * Reference relative to which to position the autocomplete panel.
   * Defaults to the autocomplete trigger element.
   */
  @Input('autocompleteConnectedTo') connectedTo!: MatAutocompleteOrigin;

  /**
   * Whether the autocomplete is disabled. When disabled, the element will
   * act as a regular input and the user won't be able to open the panel.
   */
  @Input({alias: 'autocompleteDisabled', transform: booleanAttribute})
  ...
  ngAfterViewInit() {
    this._initialized.next(undefined as void);
  ...
  private _subscribeToClosingActions(): Subscription {
    const initialRender = new Observable(subscriber => {
      afterNextRender(
        () => {
          subscriber.next(undefined as void);
        },
  ...
  private _getOverlayConfig(): OverlayConfig {
    const scrollStrategy = createRepositionScrollStrategy(this._injector);
